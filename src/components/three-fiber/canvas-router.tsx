      ...
      >
        <Canvas flat={true} shadows style={{overflow: 'visible'}}>
          ...
          <KeyboardBG
            ...
            color={accentColor}
            visible={!hideTerrainBG && !isConfigure}
          />
...
// Tide49: 3D 模式下的 6 层全展开（2列3行网格）
const AllLayers3DGroup = React.memo((props: any) => {
  const {loadProgress, configureKeyboardIsSelectable, allKeymaps} = props;
  ...
  return (
    <>
      {layers.map((layer) => {
        const col = layer.index % 2;
        const row = Math.floor(layer.index / 2);
        const xPos = (col - 0.5) * colSpacing;
        const yPos = -row * rowSpacing;

        return (
          <React.Fragment key={layer.index}>
            <Html center position={[xPos, yPos + 8, -19]}>
              <div style={{
