...
import {
  TIDE49_DEFINITION,
  TIDE49_PATH,
  TIDE49_VENDOR_PRODUCT_ID,
  TIDE49_LAYER_COUNT,
  TIDE49_KEY_COUNT,
} from '../utils/tide49-definition';
...
// Tide49: 跳过 USB 设备搜索，直接加载本地配置
export const initTide49 = (): AppThunk => async (dispatch, getState) => {
  const state = getState();
  // 如果已经初始化过，跳过
  if (state.definitions.definitions[TIDE49_VENDOR_PRODUCT_ID]) {
    return;
  }

  // 1. 加载 Tide49 定义到 definitions store
  dispatch(
    updateDefinitions({
      [TIDE49_VENDOR_PRODUCT_ID]: {
        v2: TIDE49_DEFINITION as any,
        v3: TIDE49_DEFINITION as any,
      },
    }),
  );
  ...
  dispatch(
    updateSupportedIds({
      [TIDE49_VENDOR_PRODUCT_ID]: {v2: true, v3: true},
    }),
  );
  ...
  const mockDevice: ConnectedDevice = {
    path: TIDE49_PATH,
    vendorProductId: TIDE49_VENDOR_PRODUCT_ID,
    productId: 0xe47a,
    vendorId: 0x342d,
    productName: 'Tide 49',
    protocol: 11,
  ...
  dispatch(
    updateConnectedDevices({
      [TIDE49_PATH]: mockDevice,
    }),
  );

  // 4. 选中 mock 设备
  dispatch(selectDevice(mockDevice));

  // 5. 设置 6 层
  dispatch({type: 'keymap/setNumberOfLayers', payload: TIDE49_LAYER_COUNT} as any);

  // 6. 预填充 6 层 keymap 数据
  for (let layer = 0; layer < TIDE49_LAYER_COUNT; layer++) {
    const keymap = new Array(TIDE49_KEY_COUNT).fill(0);
    dispatch({
      type: 'keymap/loadLayerSuccess',
      payload: {
        layerIndex: layer,
        keymap,
        devicePath: TIDE49_PATH,
      },
    } as any);
  }
};
